...
//	鼠标移出下拉框1秒后下拉框消失
	...
	// 柱状图横坐标全称显示
	$('.course_full').hide();
	$('.course').stop().mouseenter(function(event) {
		var len_course=$(this).find('a').text().length;
		var text_course=$(this).find('a').text();
		if(len_course>5){
			$(this).siblings('.course_full').show().find('a').text(text_course);
...
//				获得followid
				...
				if (str_type.search("211") >= 0) arr_type[0]=211;
				if (str_type.search("985") >= 0) arr_type[1]=985;
				if (str_type.search("34") >=0) arr_type[2]=34;
				// 插入学校页面头专业信息
				base_info(baseInfo.logo,baseInfo.majorName,baseInfo.school,arr_type[0],arr_type[1],arr_type[2],baseInfo.collegeId);
				...
				// 插入三个排名标签
				ranking_charu(data.rankInfo.majorRank,rank_tag1,rank_item1);
				...
				if(ratae_value>=100)
				{
					...
					applyexceed(data.applyAdmitInfo.applyCount,data.applyAdmitInfo.admitCount,data.applyAdmitInfo.exemptionCount);
				}else if(data.applyAdmitInfo.rate=='-1'){
					$('.exceed').css('display','block');
					applyexceed(data.applyAdmitInfo.applyCount,data.applyAdmitInfo.admitCount,data.applyAdmitInfo.exemptionCount,data.applyAdmitInfo.applyDescription,data.applyAdmitInfo.admitDescription);
					$('.biao_6').hide();
				}else{
					applyAdmit(data.applyAdmitInfo.rate);
					applyexceed
						(
						data.applyAdmitInfo.applyCount,
				...
				function float_value(x_unknow)
				{
					...
					{
						maininfo[x_unknow].value=parseFloat(maininfo[x_unknow].value.replace('%',''));
						return maininfo[x_unknow].value;
					}
				...
				for(var i=0;i< maininfo.length;i++)
				{
					...
					columnchart(i,float_value(i),maininfo[i].majorName,maininfo[i].color,maininfo[i].schoolName);
				}
				...
				bar_tag(4,data.majorRecommendInfo.transdisciplinaryCount);
				// 感兴趣的专业
				var maininfo_major=data.interestedMajorInfo.mainInfo;
				for(i=0;i<maininfo_major.length;i++)
				{
					inster_major(i,maininfo_major[i].name,maininfo_major[i].school,maininfo_major[i].degree,maininfo_major[i].majorId);
				}

				var maininfo_collage=data.interestedCollegeInfo.mainInfo;
				for(i=0;i<maininfo_collage.length;i++)
				{
					inster_collage(i,maininfo_collage[i].name,maininfo_collage[i].level,maininfo_collage[i].rank,maininfo_collage[i].collegeId);
				}				
...
//nav01页面头信息
function base_info(img_logo,majorname,schoolname,type_211,type_985,type_34,collageid)
{
	console.log(img_logo);
	$('.sch_name').find('img').attr('src','http://www.itui.cn/itui/images/'+img_logo);
...
var rank_item3='院校省内排名';
 ...
 {
 var ranking='<p class="rank_p">'+rank_value+'</p><p class="pai">'+rank_item+'</p>';
...
// 近三年报录比均值
function applyAdmit(rate_percent){
...
function columnchart(bar_index,column_height,majorname,bar_color,schoolname)
{
	...
	{
		$('.bar_cha'+(bar_index+1)).css('height', column_height*1.5+'px');
...
function inster_major(major_index,major_name,collage_name,exam_degree,maior_id)
{
	$('.major0 li').eq(major_index).find('.zy_name'+(major_index+1)).text(major_name);
	$('.major0 li').eq(major_index).find('.yuanxi'+(major_index+1)).text(collage_name);
	$('.major0 li').eq(major_index).find('.nandu'+(major_index+1)).text(exam_degree);
	// 点击专业跳转
	$('.major0 li').eq(major_index).click(function(event) {
		thisid=maior_id;
		$.cookie("thisid", thisid,{path:"/"});
		window.open("info.html?mid="+thisid,"_blank");
		console.log('新ddd的'+thisid);
...
var collageleve='';
var collagerank='';
function inster_collage(collage_index,collage_name,collage_leve,collage_rank,collage_id)
{
	$('.collage0 li').eq(collage_index).find('.xx_name'+(collage_index+1)).text(collage_name);
	if(collage_leve=='null')
	{
	$('.collage0 li').eq(collage_index).find('.cci_'+(collage_index+1)).text('其他');
	}else
	{
	$('.collage0 li').eq(collage_index).find('.cci_'+(collage_index+1)).text(collage_leve);
	}
	
	$('.collage0 li').eq(collage_index).find('.pai_'+(collage_index+1)).text(collage_rank);
	// 点击学校跳转
	$('.collage0 li').eq(collage_index).click(function(event) {
		...
		$.cookie("thiscid", thiscid,{path:"/"});
		$.cookie("collagename", collagename,{path:"/"});
		$.cookie("collageleve", collageleve,{path:"/"});
		$.cookie("collagerank", collagerank,{path:"/"});
		window.open("school.html?cid="+thiscid,"_blank");
