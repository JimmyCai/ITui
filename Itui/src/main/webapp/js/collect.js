...
major_name="计算机及应用";
var listsch_htmlcode='<div class="sch_part sch_part0'+id_ul+'">\
			<div class="item0 item0'+id_ul+'" new_cid="'+collage_id+'">\
				<div class="btn_and btn_and0'+id_ul+'">+</div>\
				<div class="btn_and btn_minus btn_minus0'+id_ul+'">-\
				</div>\
				<p class="schname schname0'+id_ul+'">'+coll_name+'</p>\
				<div class="total">\
				<span>(</span>\
				<span class="total_num total_num0'+id_ul+'">'+follow_count+'</span>\
				<span>)</span>\
				</div>\
				<div class="sch_delete sch_delete0'+id_ul+'" title="删除收藏学校">×</div>\
			</div>\
			<ul class="ul0 ul0'+id_ul+' ul_height">\
			</ul>\
		     </div>';
...
function listmajor_insert(ul_id,li_id,major_name,follow_count,major_id)
{
		...
		var listmajor_htmlcode='\
			<li class="li_list lilist_0'+li_id+'" new_mid='+major_id+'>\
		                  <p>'+major_name+ul_id+li_id+'</p>\
		                  <div class="major_delete major_delete0'+ul_id+li_id+'" title="删除收藏专业">×</div>\
 		             </li>';
...
$.ajax({
	...
	type: 'post',
	dataType: 'html',
	data: {'code': 'f4071ec84347304e83512c25f8af4f67'},
	...
	if(status==0)
	   { 
		...
		listsch_insert(j,schlist[j].collegeName,schlist[j].followMajorCount,collage_id);
		// 展开收起控制按钮
		schlist_control(j);
		// 删除收藏学校
		delete_sch(j);
		...
		listmajor_insert(j,w,major_name,follow_count,major_id);
...
$.ajax({
	// url: 'http://42.96.190.127:8080/Itui/getfollowmajor.html',
	url: 'http://42.96.190.127:8080/Itui/disfollowcollege.html?msg'+Math.random(),
	type: 'post',
	dataType: 'html',
	data: {'id': cid},
	success: function (msg)
...
// 传送用户删除的专业ID
function post_mid(mid)
{
$.ajax({
	// url: 'http://42.96.190.127:8080/Itui/getfollowmajor.html',
	url: 'http://42.96.190.127:8080/Itui/disfollowmajor.html?msg'+Math.random(),
	type: 'post',
	dataType: 'html',
	data: {'id': mid},
	success: function (msg)
