...
// 定义注册对象
var forgetregs_obj={objemail:'null',objpasd:'null'};
// 判断注册对象是不是null
// 注册邮箱验证开始
	function forget_email()
	{
	$('.forget_email').blur(function(event) 
	{
		var email_forget=$('.forget_email').val();
		var reg =/^([a-zA-Z0-9_-])+@([a-zA-Z0-9_-])+((\.[a-zA-Z0-9_-]{2,3}){1,2})$/;
		if(reg.test(email_forget))
		{
		$('.err_prompt01').html('√邮箱合法').css('color', '#6fd415');
		// 将正确的邮箱存进注册对象
		forgetregs_obj.objemail=email_forget;
		// 判断是否两个输入框都合法
		forget_submit_judge(forgetregs_obj.objemail,forgetregs_obj.objpasd);
		}else
		{
				
		$('.err_prompt01').html('×邮箱不合法').css('color', 'red');
		forgetregs_obj.objemail='null';
		}
		if(email_forget=='')
		{
		$('.err_prompt01').html('×邮箱不能为空').css('color', 'red');
		forgetregs_obj.objemail='null';
		}
	});
	}
...
// 注册密码验证
	...
	$('.forget_pasd').blur(function(event)
	 {
		...
		// 注册确认密码
			$('.sure_spasd').blur(function(event)
			 {
				
				var forgetpasd3=$('.sure_spasd').val();
				if(forgetpasd2==forgetpasd3)
				{
				$('.err_prompt03').html('√密码一致').css('color', '#6fd415');
				// 将正确的密码存进注册对象
				...
				{
				$('.err_prompt03').html('×两次密码输入必须一致').css('color', 'red');
		...
		{
		$('.err_prompt02').html('×输入6-15位以字母开头含有字母数字的密码').css('color', 'red');
		forgetregs_obj.objpasd='null';
		}
		if(forgetpasd2=='')
		{
...
if(status==0)
   {
   	...
   	{
	$('.forget_div h3').text(data.normalReturn.msg+'请重新注册').css('color', 'red');
   	}else
   	{
   	// 登录成功后提醒用户去邮箱查看邮件
   	$('.forget_form').hide();
   	...
   	$('.info_prompt p').text('先去邮箱查看邮件激活吧!').css('color', 'red');
   	console.log(pasd);
     	console.log(email);
